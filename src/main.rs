...
fn draw_lane_markings() {
    ...
    let intersection_start_x = center_x - ROAD_WIDTH / 2.0;
    let intersection_end_x = center_x + ROAD_WIDTH / 2.0;
    let intersection_start_y = center_y - ROAD_WIDTH / 2.0;
    let intersection_end_y = center_y + ROAD_WIDTH / 2.0;

    let mut current_x = 0.0;
    while current_x < intersection_start_x {
        let end_x = (current_x + dash_length).min(intersection_start_x);
        draw_line(current_x, center_y, end_x, center_y, 2.0, YELLOW);
        current_x += segment_length;
    }

    current_x = intersection_end_x;
    while current_x < screen_width() {
        let end_x = (current_x + dash_length).min(screen_width());
        draw_line(current_x, center_y, end_x, center_y, 2.0, YELLOW);
        current_x += segment_length;
    }
    ...
    while current_y < intersection_start_y {
        ...
        draw_line(center_x, current_y, center_x, end_y, 2.0, YELLOW);
        current_y += segment_length;
    }

    current_y = intersection_end_y;
    while current_y < screen_height() {
        let end_y = (current_y + dash_length).min(screen_height());
        draw_line(center_x, current_y, center_x, end_y, 2.0, YELLOW);
        current_y += segment_length;
    }
}
